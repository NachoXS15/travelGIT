...
export default function Card({ destino, salida, categoria, imgUrl }: PackageProps) {
  return (
    <div style={{ fontFamily: 'Mundial' }} className="flex justify-between flex-col w-full rounded-lg sm:h-fit md:h-96 shadow-2xl p-5 bg-lightgray md:w-72 hover:border-bluemain">
      ...
      <button className="w-fit bg-bluesec mt-5 rounded-full py-1 px-4 text-white hover:border-bluemain hover:bg-white hover:text-bluesec transition duration-300">Ver Más</button>
    </div>
  )
}
